...
% ========== recipeingredients environment ==========
% Starts a single tabular covering the whole recipe ingredient block
\RenewDocumentEnvironment{recipeingredients}{}
{
    \vspace*{-0.01\textheight}% your spacing (optional)
    \firstingredientsectiontrue
    \begin{tabular}{r @{\hspace{1em}} p{4cm}}
}
{
    \end{tabular}
}
...
% ========== ingredientsection ==========
% Prints a header row inside the tabular
\RenewDocumentCommand{\ingredientsection}{m}{%
    % Insert space before all but first section
    \iffirstingredientsection
        \firstingredientsectionfalse
    \else
        \\[0.8em]  % vertical space between sections
    \fi
...
% Adds a normal ingredient row
\RenewDocumentCommand{\ingredient@formatmulti}{m m m m}{%
    % Trim values first
    \def\ingredient@qty{#2}\trim@spaces@in\ingredient@qty%

    % Output the accumulated main part
    \textit{\small \ingredient@qty} & \textit{\small \ingredient@rightside{#1}{#3}{#4}} \\%
}

% Adds a normal ingredient row
\NewDocumentCommand{\ingredient@rightside}{m m m}{%
    #2\ #1\ #3
}
